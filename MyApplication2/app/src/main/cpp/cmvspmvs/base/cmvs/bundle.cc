...
void Cbundle::prep(const std::string prefix, const int imageThreshold,
  ...
  if (pnumThreshold != 0) {
    ...
    LOGI("Should use pnumThreshold = 0");
    exit (1);
  }
...
void Cbundle::prep2(void) {
  // Used in mergeSfMP now.
  {
    ...
    LOGI("done use XX secs");
    startTimer();
    cerr << "slimNeighborsSetLinks..." << flush;
    LOGI("slimNeighborsSetLinks...");
    slimNeighborsSetLinks();
    cerr << "done\t" << curTimer() << " secs" << endl;
    LOGI("done %d secdd",curTimer());
  }
...
void Cbundle::run(const std::string prefix, const int imageThreshold,
  ...
  cerr << "done\t" << curTimer() << " secs" << endl;
  LOGI("READ IMAGE DONE!");
  
  m_oimages.resize((int)m_timages.size());
  LOGI("writeCameraCenters");
  writeCameraCenters();
  LOGI("writeVis....");
  // Output results
  writeVis();
...
void Cbundle::readBundle(const std::string file) {
 ...
 // LOGI("  focal lengths: %0.3f, %0.3f\n", cameras[0].f, cameras[1].f);
  LOGI("camera-- %d have  %d point in bundle file\n",cnum,pnum);
  m_cnum = 0;
  ...
  LOGI(" %d cameras %d points",m_cnum,m_pnum);
}
