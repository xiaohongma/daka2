...
void read_ply_file ( char* infile );
void write_obj_file ( char* outfile );
...
*/
{
  ...
  while ( --argc > 0 && (*++argv)[0] == '-' ) 
  {
    ...
    LOGI ( "start read ply");
  read_ply_file ( infile);
    LOGI ( "read ply end" );
    LOGI ( "start write obj" );
  write_obj_file (outfile );
    LOGI ( "write obj end" );

  LOGI (  "\n" );
  LOGI ("PLY_2_OBJ:\n" );
  LOGI ("  Normal end of execution.\n" );
...
*/
{
  LOGI ("usage: %s [flags] <in.ply >out.obj\n", progname );
  return;
}
/******************************************************************************/
...
*/
{
 ...
 FILE *fp;
  ...
  fprintf ( fp,"# OBJ file created by ply_to_obj.c\n");
  fprintf(fp, "#\n");
/*
...
*/
  ...
  for ( i = 0; i < nfaces; i++ ) 
  {
    ...
    {
      fprintf(fp, "f  %d", ( flist[i]->verts[k+1] ) + 1 );
      fprintf(fp, "  %d", ( flist[i]->verts[k] ) + 1 );
      fprintf(fp, "  %d\n", ( flist[i]->verts[0] ) + 1 );
    }
  }
    fclose(fp);
