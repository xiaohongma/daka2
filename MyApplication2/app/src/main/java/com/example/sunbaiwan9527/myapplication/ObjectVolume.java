...
public class ObjectVolume {
	...
	public static void caculateVolume(String obj_path) {
		...
		try {
			...
			Log.i(TAG,"   Total Volume       =   %13.5f\n"+ totalVolume);
			Log.i(TAG,"   Total Surface Area =   %13.5f\n"+totalArea);
			cx = cx / totalVolume;
			cy = cy / totalVolume;
			cz = cz / totalVolume;
			System.out
			...
			Log.i(TAG,"   Total Number of Faces =     %8d\n\n"+fCount);
			Log.i(TAG,"         Number of Sides   Count");
			for (int i = 0; i < 50; i++) {
				if (faces[i] > 0.0) {
					Log.i(TAG,"                 " + i + "         "
							+ faces[i]);
			...
			Log.i(TAG,"\n   Total Number of Triangles Processed = "
					+ triangleCount);
			Log.i(TAG,"   Total Number of Undefined Volumes = "
					+ NaNCount);
			time2 = System.currentTimeMillis();
			Log.i(TAG,"\n   Total Program Run Time = "
					+ (time2 - time1) + " milli seconds");
			Log.i(TAG,"   Time in Calc Method = " + (time4 - time3)
					+ " milli seconds");
			in.close();

		} catch (IOException e) {
			Log.i(TAG,"File Not Found: " + fileIn);
		}
	}
